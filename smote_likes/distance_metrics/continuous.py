...
from .categorical import get_cond_probas, _remap_ndarray_dict, ShapeError
...
def interpolated_vdm(X: numpy.ndarray, y: numpy.ndarray, s: int) -> numpy.ndarray:
    ...
    for trgt in unique_targets:
        ...
        for col in range(X_discrete.shape[1]):
            z_a = _get_interpolated_location(
                X[:, col], X_discrete[:, col], widths[0, col], widths[2, col])
            P_a_u0_c = _remap_ndarray_dict(
                X_discrete[:, col], cond_proba_list[col][trgt])
            P_a_u1_c = _remap_ndarray_dict(
                X_discrete[:, col] + 1, cond_proba_list[col][trgt])
